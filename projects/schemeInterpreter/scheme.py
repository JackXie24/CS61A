...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        return scheme_eval(complete_eval(no_eval), env)
        # END Problem 21
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        symb_pair= Pair(bindings.first.first, symb_pair)
        val_pair= Pair(scheme_eval(bindings.first.second.first, env), val_pair)
        bindings= bindings.second
    check_formals(symb_pair)
    return env.make_child_frame(symb_pair, val_pair)
    # END PROBLEM 15
