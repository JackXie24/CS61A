...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 5
    f, g = say_scores, announce_lead_changes
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def high_score(score0, score1):
        if who == 0:
            highscore, current = score0 - previous_score, score0
            if highscore > previous_high:
                if highscore == 1:
                    print("1 point! That's the biggest gain yet for Player 0")
                else:
                    print(highscore, "points! That's the biggest gain yet for Player 0")
                return announce_highest(who, highscore, current)
        else:
            highscore, current = score1 - previous_score, score1
            if highscore > previous_high:
                if highscore == 1:
                    print("1 point! That's the biggest gain yet for Player 1")
                else:
                    print(highscore, "points! That's the biggest gain yet for Player 1")
                return announce_highest(who, highscore, current)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    k, current_num, highest_num_roll = 1, 0, 0
    while k < 11:
          next_num = make_averaged(roll_dice, num_samples) (k, dice)
          if current_num < next_num:
              highest_num_roll, current_num = k, next_num
          k += 1
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if is_swap(score1, opponent_score) is True or margin <= free_bacon(opponent_score):
        return 0
    else:
        return num_rolls
